...
| CPA / Grok Build | 无完整 mint 流水线 | OAuth mint → `cpa_auths/xai-*.json` |
...
- 成功账号实时写入 `accounts_*.txt` / `accounts_cli.txt`
...
- 不要提交或泄露：`config.json`、API Key、JWT、Cookie、密码、SSO Token、代理凭据、账号文件、`cpa_auths/`、管理密钥
