...
- `register_cli.py` / `grok_register_ttk.py` / `cf_mail_debug.py`：根目录**薄入口**（兼容旧命令，内部转到包）。
- `grok_register/`：**核心包**
  - `app.py`：原 `grok_register_ttk.py`（GUI + 注册主逻辑 + 兼容 CLI）
  - `cli.py`：原 `register_cli.py`（多线程注册 + 异步 mint）
  - `tab_pool.py`、`cpa_export.py`、`cpa_to_sub2api.py`、`cf_mail_debug.py`
  - `paths.py`：项目根 / `config.json` / `output/` / `turnstilePatch/` 路径锚点
  - `cpa_xai/`：Grok Build OAuth mint
- `output/`：运行产物（账号、CPA、token、导出）；默认勿提交。
- `scripts/`：回填、校验、启动辅助。
- `docs/`：本地说明（非主文档）。
- `tests/`：标准库 `unittest`。
- `turnstilePatch/`：Chromium 扩展，**必须留在项目根**，不要移动或删除。
...
# 在已有账号文件上追加
uv run python -u register_cli.py --extra 50 --threads 2 --accounts-file .\output\accounts_batch100.txt
...
- `grok_register/app.py` 体积大、全局状态多，修改需谨慎。
- 除非任务要求配置迁移，保持现有字段与默认值兼容。
- 新增配置项：更新代码默认值、`config.example.json`、`README.md`（及本文件要点）。
- 改注册公共逻辑时同时考虑 GUI 与 CLI（尤其 `grok_register/cli.py` 的 mint 流水线）。
...
- 敏感运行数据（勿提交、勿在回复中完整粘贴）：
  ...
  - `output/accounts_*.txt`、`output/accounts_cli.txt`（根目录旧文件也应忽略）
  - `output/mail_credentials.txt`
  - `output/cpa_auths/`（及旧路径 `cpa_auths/`）
