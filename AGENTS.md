...
- **操作手册（完整）**：`README.zh.md` — 安装、必须配置、GUI/CLI 启动、多线程批量命令、CLI 参数、输出文件、FAQ。改运行说明时优先同步这里。
- **英文 README**：`README.md` — 可能偏旧/不全；顶部已链到中文说明。以 `README.zh.md` 与本文件为准。
- **配置样例**：`config.example.json` — 与代码默认值保持一致。
- 添加配置键时：同步更新 **代码默认值**、`config.example.json`、`README.zh.md`（以及本文件中相关要点）。
...
- 必须使用 Python **3.13**。`pyproject.toml` 要求 `>=3.13,<3.14`。README 英文徽章若写 3.9+ 已过时。
...
细节与 JSON 示例见 `README.zh.md`「必须配置」章节。
...
- 新增配置项：更新代码默认值、`config.example.json`、`README.zh.md`（及本文件要点）。
